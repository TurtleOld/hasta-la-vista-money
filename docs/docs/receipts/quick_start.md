...
#### Вариант C: Сканировать QR камерой

1. **Нажмите "Обработка изображения чека"**
2. Откройте вкладку **"Сканировать QR"**
3. **Выберите счёт** для списания
4. Разрешите браузеру доступ к камере
5. Поместите QR-код целиком внутрь рамки и держите телефон ровно
6. При слабом освещении включите фонарик, а для мелкого QR используйте
   приближение, если эти функции доступны на устройстве
7. После распознавания чек автоматически отправится в обработку

Сканирование выполняется непосредственно в современном браузере. Кадры с
камеры не отправляются на сервер: отправляется только содержимое распознанного
QR-кода. Если камера или сканер недоступны, используйте вкладку **"Файл"**.
...
- При сканировании камерой держите QR целиком внутри рамки так, чтобы он занимал
  примерно 30–70% её ширины
- Если QR не распознан за несколько секунд, включите фонарик, измените
  приближение или коснитесь изображения для фокусировки
