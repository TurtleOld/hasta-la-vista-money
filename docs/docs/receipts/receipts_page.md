...
Модуль чеков предоставляет комплексное решение для управления кассовыми чеками, включая загрузку QR-кодов с получением официальных данных из ФНС, ручное добавление и детальную аналитику покупок.

!!! abstract "Основные возможности"

    - **Загрузка QR-кода чека** с получением данных из ФНС
...
!!! abstract "Доступные действия"
    ...
    - **Обработка изображения чека** - QR-flow с интеграцией ФНС
...
- **НДС** - разбивка по ставкам 10% и 22%
...
## Обработка чеков по QR-коду

!!! abstract "QR-flow"

    Система извлекает QR-код из изображения и получает данные чека через API ФНС.

На странице загрузки чека доступны две вкладки:

- **Файл** — загрузка готового изображения (фото) с QR-кодом чека
- **Сканировать QR** — распознавание QR-кода в реальном времени через камеру устройства

### Сканирование через вкладку «Сканировать QR»

1. Откройте вкладку **«Сканировать QR»** и разрешите доступ к камере
2. Наведите камеру на QR-код чека — распознавание происходит автоматически
3. Если фокус не срабатывает, коснитесь изображения в месте расположения QR-кода
4. После распознавания данные чека отправляются на обработку автоматически

!!! info "Автофокус и зум"

    При запуске камеры система автоматически включает непрерывный автофокус и
    выставляет стандартный зум (1x). Это важно для телефонов с несколькими
    камерами: без этого браузер может выбрать сверхширокоугольный объектив,
    из-за чего изображение выглядит сильно отдалённым и размывается при
    приближении к чеку.
...
1. **Загрузите изображение** с QR-кодом чека
2. **Выберите счет** для списания
3. **Нажмите "Загрузить"**
4. **Дождитесь обработки** (QR -> ФНС -> маппинг)
5. **Проверьте результат** и при необходимости отредактируйте

### Какие данные заполняются автоматически

Из ответа ФНС система заполняет:
...
!!! tip "Качество QR-кода"

    Для лучшего результата используйте четкие снимки QR-кода без бликов и размытия.
...
#### Камера отдаляет изображение или размывает вблизи

**Причина:** На телефонах с несколькими камерами браузер может выбрать
сверхширокоугольный объектив или минимальный зум. Такой объектив не умеет
фокусироваться на близком расстоянии, поэтому QR-код выглядит отдалённым,
а при приближении — размытым.

**Решение:**

- Система автоматически выставляет стандартный зум (1x) при запуске камеры —
  обновите страницу и откройте вкладку «Сканировать QR» заново
- Коснитесь изображения в месте QR-кода, чтобы принудительно сфокусироваться
- Если изображение всё равно отдалено или размыто (браузер не поддерживает
  управление зумом), сфотографируйте чек штатной камерой телефона и загрузите
  фото через вкладку «Файл»

#### "Ошибка при обработке изображения"

**Причина:** Проблемы с качеством QR-кода или недоступность API ФНС.
...
def process_receipt_image(image_file):
    try:
        result = process_pending_receipt(image_file)
