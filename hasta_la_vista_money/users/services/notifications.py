...
def get_user_notifications(user: User) -> list[NotificationDict]:
    ...
    low_balance_accounts = [
        acc
        for acc in accounts
        if float(acc.balance) < constants.LOW_BALANCE_THRESHOLD
    ]
        ...
        notifications.append(
            {
                ...
                'message': (
                    f'В текущем месяце расходы превышают доходы на '
                    f'{current_month_expenses - current_month_income:.2f} ₽'
                ),
    ...
    if (
        ...
        and (current_month_income - current_month_expenses)
        / current_month_income
        > constants.SAVINGS_THRESHOLD
    ):
        notifications.append(
            {
                'type': 'success',
                ...
                'message': (
                    'Вы сэкономили более 20% от доходов в текущем месяце'
                ),
