...
_CATEGORY_RULES: Final[tuple[tuple[str, tuple[str, ...]], ...]] = (
    ...
    ('Хлеб и выпечка', ('хлеб', 'батон', 'булк', 'лаваш', 'багет')),
    ('Мясо и птица', ('куриц', 'говяд', 'свинин', 'фарш', 'колбас', 'сосиск')),
    (
        'Рыба и морепродукты',
        ('рыб', 'лосос', 'семг', 'сельд', 'тунец', 'кревет'),
    ),
    ('Напитки', ('вода', 'сок', 'чай', 'кофе', 'напиток', 'лимонад')),
    ('Бакалея', ('рис', 'греч', 'макарон', 'мука', 'сахар', 'соль', 'круп')),
    ('Сладости и десерты', ('шоколад', 'конфет', 'печень', 'вафл', 'морожен')),
    ('Бытовая химия', ('порошок', 'средство', 'гель', 'мыло', 'шампун')),
    ('Гигиена и косметика', ('зубн', 'паста', 'щетк', 'салфет', 'туалет')),
)
...
class ReceiptItemCategoryService:
    ...
    def categorize(self, *, user: User, product_name: str) -> str:
        ...
        if not normalized_name:
            return constants.DEFAULT_PRODUCT_CATEGORY
        ...
        return (
            self._rule_category(normalized_name)
    ...
    def _history_categories(self, user: User) -> dict[str, str]:
        rows = (
            ...
            .order_by('-created_at')
            .values_list('product_name', 'category__name')[:_HISTORY_LIMIT]
        )
